...
impl Manager {
    ...
    pub async fn alloc_service(&mut self, srv: &Service) {
        ...
        for name in srv_info.topo_order.iter() {
            let val = name_to_vals.get(name)
            .expect(&format!("Service alloc: var/def is not initialized: {}", name));
    ...
    pub async fn alloc_def_actor(&mut self, name: &String, val: Expr) -> Result<(), Box<dyn Error>> {
        let actor_ref = spawn(DefActor::new(name.clone(), val));
        self.defname_to_actors.insert(name.clone(), actor_ref.clone());
        
        // subscribe to its dependencies 
        let depends = self.dep_graph.get(name)
        .expect(&format!("Service alloc: no such def with name: {} in dependency graph", name));

        for dep in depends.iter() {
            // synchronously wait for response
            let back_msg = self.ask_to_name(dep, 
                Msg::Subscribe {
