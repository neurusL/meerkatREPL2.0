...
impl kameo::prelude::Message<Msg> for Manager {
    ...
    async fn handle(
        ...
        match msg {
            ...
            Msg::UsrReadVarResult { txn, name, result, pred } => {
            ...
            Msg::UsrReadDefResult { txn, name, result, preds } => {
            ...
            Msg::Change { from_name, val, preds } => todo!(),
...
impl Manager {
    pub async fn tell_to_name(&self, 
        name: &String, 
        msg: Msg,
    ...
    pub async fn tell_to_name2(&self, 
        name: &String, 
        msg_var: Msg,
        msg_def: Msg
    ) -> Result<(), Box<dyn Error>> {
